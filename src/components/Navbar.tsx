            ...
            <div className="hidden md:flex items-center space-x-8">
              {navLinks.map((link) => (
                <button
                  ...
                  className="text-sm font-medium transition-colors hover:text-primary-600 text-neutral-800"
                >
                  {link.name}
                </button>
              ))}
            ...
            <div className="flex flex-col space-y-4">
              {navLinks.map((link) => (
                <button
                  key={link.name}
                  onClick={() => handleLinkClick(link.href, link.external)}
                  className="text-sm font-medium text-neutral-800 hover:text-primary-600 text-left transition-colors"
                >
                  {link.name}
                </button>
              ))}
              <button
                onClick={handleBookCall}
                className="px-4 py-2 text-sm font-medium text-center rounded-md bg-primary-600 text-white hover:bg-primary-700 transition-colors"
              >
                Book a Call
              </button>
            </div>
